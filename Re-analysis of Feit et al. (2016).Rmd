...
assign(curr_plot, curr_lm %>%
  ...
  mutate(dist_mdn = eval(as.name(x))-mean(eval(as.name(x)))) %>%
    ggplot(aes(x=source, y =dist_mdn, group = source, colour=cond, alpha=0.7))+
    geom_violin(alpha=0.4)+
    geom_beeswarm(size = 2, cex=2, alpha= 0.5)
}
else{
...
plots <- c("1", "2")
attr <- list("mean_wmp_per_person", "mean_sd_iki_per_person", "mean_uer_per_person", "mean_ke_per_person")
