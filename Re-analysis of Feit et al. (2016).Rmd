...
for (var in vars){
  ...
  for (path in eval(parse(text=var))){
    ...
    #colnames(nam) <- lapply(nam[1, ], as.character)
    #nam <- nam[-1,]
    }
    else{
...
assign(tt, currnam[currnam$user_id %in% c(touch_typist_id),])
 assign(ntt, filterntt <- currnam[currnam$user_id %in% c(non_touch_typist_id),]) 
filtertt <- t2[t2$user_id %in% c(touch_typist_id),]
  i <- i + 1
  
}

#new_nam <- paste(x, "one", sep="_")
  #assign(new_nam, x %>% group_by(user_id, current_input) %>% summarise(m = mean(as.numeric(wpm))))
